...
class ServerTunnel: Tunnel, TunnelDelegate, StreamDelegate {
    ...
    class func startListeningOnPort(port: Int32) -> NetService {
        let service = NetService(domain:Tunnel.serviceDomain, type:Tunnel.serviceType, name: Tunnel.serviceName, port: port)

		simpleTunnelLog("Starting network service: \(service) on port \(port)")
		...
		repeat {
			...
			if packetBytesRemaining == 0 {
				...
				if packetBuffer.count == MemoryLayout<UInt32>.size {
                    ...
                    packetBuffer.copyBytes(
                        to: withUnsafeMutablePointer(to: &totalLength, {
                            let rawPointer = UnsafeMutableRawPointer($0)
                            return rawPointer.assumingMemoryBound(to: UInt8.self)
                        }),
                        count: MemoryLayout.size(ofValue: totalLength)
                    )
	...
	func sendOpenResultForConnection(connectionIdentifier: Int, resultCode: TunnelConnectionOpenResult) {
        let properties = createMessagePropertiesForConnection(
            connectionIdentifier,
            commandType: TunnelCommand.openResult,
            extraProperties: [TunnelMessageKey.ResultCode.rawValue : resultCode.rawValue] as [String: AnyObject]
        )
    ...
    override func handleMessage(_ command: TunnelCommand, properties: [String : AnyObject], connection: Connection?) -> Bool {
        switch command {
        case .open:
        ...
        case .fetchConfiguration:
            ...
            let messageProperties = createMessagePropertiesForConnection(
                0, 
                commandType: .fetchConfiguration,
                extraProperties: [TunnelMessageKey.Configuration.rawValue: personalized] as [String : AnyObject]
            )
...
class ServerDelegate : NSObject, NetServiceDelegate {
    ...
    func netService(_ sender: NetService, didAcceptConnectionWith inputStream: InputStream, outputStream: OutputStream) {
        simpleTunnelLog("Accepted a new connection with input: \(inputStream) output: \(outputStream)")
