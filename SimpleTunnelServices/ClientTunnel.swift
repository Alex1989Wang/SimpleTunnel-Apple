...
open class ClientTunnel: Tunnel {
    ...
    private(set) var connectionStatusObservation: NSKeyValueObservation?
	...
	open func startTunnel(_ provider: NETunnelProvider) -> SimpleTunnelError? {
        ...
        connectionStatusObservation = connection.observe(\.state, options: .initial, changeHandler: { 
            [weak self] connection, change in
            guard let self else {
                return
            }
            simpleTunnelLog("Connection state changed: \(connection.state.description) error: \(String(describing: connection.error))")
            ...
            case .cancelled:
                self.connection = nil
                self.connectionStatusObservation = nil
	...
	open func sendMessage(_ messageProperties: [String: AnyObject], completionHandler: @escaping (Error?) -> Void) {
        ...
        connection?.write(messageData, completionHandler: { error in
            simpleTunnelLog("Send message: \(messageProperties) error: \(String(describing: error))")
